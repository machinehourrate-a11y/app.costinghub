...
import { suggestMultipleTools } from '../services/geminiService';

type ToolSuggestion = Omit<Tool, 'id' | 'user_id' | 'created_at'>;

interface MultiToolModalProps {
  onSave: (tools: ToolSuggestion[]) => void;
  onClose: () => void;
}

export const MultiToolModal: React.FC<MultiToolModalProps> = ({ onSave, onClose }) => {
  ...
  const handleGenerate = async () => {
    ...
    setSelectedIndices([]);
    try {
      const result = await suggestMultipleTools(prompt);
      if (result) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in">
      <Card className="max-w-3xl w-full relative">
        ...
        <h2 className="text-2xl font-bold text-primary mb-4">Add Multiple Tools with AI</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder="e.g., 'A set of 4-flute carbide end mills for aluminum: 6mm, 10mm, and 12mm' or 'HSS drills for steel: 3mm and 5mm'"
              ...
              <table className="min-w-full divide-y divide-border">
                <thead className="bg-background/50">
                    <tr>
                        ...
                        <th className="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase">Brand</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase">Model</th>
                ...
                <tbody className="bg-surface divide-y divide-border">
                    {suggestions.map((item, index) => (
                        <tr key={index} className={`hover:bg-background/60 ${selectedIndices.includes(index) ? 'bg-primary/10' : ''}`}>
                            ...
                            <td className="px-4 py-2 text-sm text-text-secondary">{item.brand}</td>
                            <td className="px-4 py-2 text-sm text-text-secondary">{item.model}</td>
                            <td className="px-4 py-2 text-sm text-text-secondary">{item.toolType}</td>
                        </tr>
                    ))}
                </tbody>
              </table>
