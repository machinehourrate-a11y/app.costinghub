...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Manually check for the recovery hash on initial page load. This is the most reliable way.
    if (window.location.hash.includes('type=recovery')) {
        isInRecoveryFlow.current = true;
    }

    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      // If the event is PASSWORD_RECOVERY, we are definitely in the flow.
      if (_event === 'PASSWORD_RECOVERY') {
          isInRecoveryFlow.current = true;
      }

      // If we have a session AND we are in the recovery flow, show the reset password page.
      // This takes precedence over a normal "signed in" state.
      ...
      // If we have a session and are NOT in recovery flow, this is a normal login/session refresh.
      if (session) {
        setSession(session);
        // We set loading true here again just in case, before fetching data.
        setLoading(true);
        fetchData(session).finally(() => {
          // Only navigate after data is fetched to avoid showing an empty dashboard.
      ...
      // If we reach here, there is no session.
      // Reset everything for the logged-out state.
      isInRecoveryFlow.current = false; // Important: reset the flag
      setSession(null);
      setUser(null);
      setCalculations([]);
      setCurrentView('auth');
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
        // If there's no session and we are not starting a recovery flow,
        // we can safely stop loading and show the login page.
    ...
    return (
      <PublicLayout theme={theme} setTheme={setTheme}>
        <ResetPasswordPage onPasswordReset={async () => {
          // Clear the recovery token from the URL to prevent re-entry on refresh.
          ...
          // Sign out. This triggers the onAuthStateChange listener, which will 
          // set the view to 'auth' and show the login page.
