    ...
    <div className="w-full mx-auto space-y-8 animate-fade-in">
        {isPricingModalOpen && selectedItem && (
            <RegionPricingModal
                ...
                regionCosts={props.countryCosts}
                regionCurrencyMap={props.countryCurrencyMap}
...
// --- Region Currency Tab ---
const RegionCurrencyTab: React.FC<Pick<CostMasterPageProps, 'countryCurrencyMap' | 'onAddCountryCurrency' | 'onDeleteCountryCurrency' | 'user'>> = ({
    countryCurrencyMap: regionCurrencyMap, onAddCountryCurrency: onAddRegionCurrency, onDeleteCountryCurrency: onDeleteRegionCurrency, user
}) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Frontend check for existing mapping for this user
        ...
        setIsSaving(true);
        try {
            await onAddRegionCurrency({ country: trimmedRegion, currency: newCurrency });
            setNewRegion('');
            setNewCurrency('');
        } catch (err: any) {
             const msg = err.message || '';
             // Handle unique constraint violations gracefully
             if (msg.includes('region_currency_map_region_key') || msg.includes('region_currency_map_user_idx') || msg.includes('duplicate key')) {
                 if (msg.includes('region_currency_map_region_key')) {
                     setError('Database Constraint Error: The database is still enforcing global uniqueness. Please ask an admin to run the "fix_constraints.sql" script.');
                 } else {
                     setError('This region already exists in your list.');
                 }
             } else {
                 setError(msg || 'An unexpected error occurred while adding the region.');
             }
        } finally {
            setIsSaving(false);
        }
