...
export const CalculatorPage: React.FC<CalculatorPageProps> = ({ 
    user, materials, machines, processes, tools, regionCosts, regionCurrencyMap, 
    onSave, onSaveDraft, onAutoSaveDraft, onBack, existingCalculation, theme, 
    onNavigate, onHeaderInfoChange, onAddTool, onShare, onRevokeShare, onGetShares 
}) => {
  ...
  const getPriceInfo = useCallback((
    ...
    itemName: string
      ...
      const costEntry = getRegionCost(region);
  ...
  useEffect(() => {
    ...
    debounceTimeout.current = window.setTimeout(async () => {
        ...
        const draftCalculation: Calculation = {
          ...
          inputs: formData,
          status: 'draft',
          user_id: existingCalculation?.user_id || user.id,
          created_at: existingCalculation?.created_at || formData.createdAt,
          isShared: existingCalculation?.isShared
        };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      onSave({
        ...
        results: results,
        status: 'final',
        user_id: existingCalculation?.user_id || user.id, // Preserve owner if editing
        created_at: existingCalculation?.created_at || formData.createdAt,
        isShared: existingCalculation?.isShared
      });
    }
  ...
  return (
    <div className="w-full mx-auto animate-fade-in pb-24 sm:pb-0">
        ...
        {isShareModalOpen && existingCalculation && (
            <ShareModal
        ...
        <div className="mb-6 flex justify-between items-center">
            ...
            {existingCalculation && !existingCalculation.isShared && (
        ...
        <main>
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <Card>
                    <h2 className="text-xl sm:text-2xl font-semibold text-primary border-b border-border pb-3 mb-6">Raw Material Geometry</h2>
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
