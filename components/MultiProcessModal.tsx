...
import { suggestMultipleProcesses } from '../services/geminiService';

type ProcessSuggestion = Omit<Process, 'id' | 'user_id' | 'created_at'>;

interface MultiProcessModalProps {
  onSave: (processes: ProcessSuggestion[]) => void;
  onClose: () => void;
}

export const MultiProcessModal: React.FC<MultiProcessModalProps> = ({ onSave, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in">
      <Card className="max-w-3xl w-full relative">
        ...
        <h2 className="text-2xl font-bold text-primary mb-4">Add Multiple Processes with AI</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder="e.g., 'Standard milling processes like face milling and end milling' or 'Drilling and tapping'"
              ...
              <table className="min-w-full divide-y divide-border">
                <thead className="bg-background/50">
                    <tr>
                        ...
                        <th className="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase">Group</th>
                    </tr>
                </thead>
                <tbody className="bg-surface divide-y divide-border">
                    {suggestions.map((item, index) => (
                        <tr key={index} className={`hover:bg-background/60 ${selectedIndices.includes(index) ? 'bg-primary/10' : ''}`}>
                            ...
                            <td className="px-4 py-2 text-sm text-text-secondary">{item.group}</td>
                        </tr>
                    ))}
                </tbody>
              </table>
